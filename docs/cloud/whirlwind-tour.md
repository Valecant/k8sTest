...
You should have your Cylcomed cluster up and running somewhere and a
shell with a `KUBECONFIG` env var pointing to the admin creds for your
cluster.
...
for Keycloak's DB and the last one for the Cylcomed MinIO tenant.
...
on the terminal. You should see the Cylcomed tenant the GitOps pipeline
